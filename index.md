...
[^n9]: Eight Guardian Defender contest submissions, all rejected. CP-006 (CLOBHelper double-rounding, identified by the harness in exploit mode and triaged by the author as Medium severity) was a ninth finding the harness discovered but the author did not submit. The original version of this footnote (paper at commit 46bb53b through 01f02ea) reported "1 accepted (CP-006)" — an affirmative-false claim corrected here, and audited as a Refuted row in §8. Per-submission rejection labels are not individually archived in public records; Guardian Defender's coordinated-disclosure terms govern any future public release.
...
We applied the §2 C1 procedure to this paper's own load-bearing claims. "Predicate availability" distinguishes rows where the §2 C1 procedure executes against the agent-authored artifact ("Refuted" / "Non-refuted") from rows where the predicate runs at scorer-tabulation only because the primary artifact was not preserved. The table below names each claim, the predicate that adjudicates it, the pointer at which the predicate executes, and its predicate availability; **two rows are refuted** (one within the §5 worked sidecar; one at the abstract-claim layer, a within-revision correction of an affirmative-false claim in the original ^n9 footnote), one is the worked non-refutation, and three are operational at the scorer-tabulation level only — primary sidecars and traces were not preserved through subsequent run archival, so the predicate cannot reach back to the agent-authored artifact layer.
...
| Abstract `^n9` original "1 accepted (CP-006)" | Author re-check against personal contest-portal submission records | Author's contest-portal history (private) — not preserved as a public artifact | **Refuted** | The original ^n9 attributed CP-006 as the 1 acceptance; CP-006 was harness-discovered but not submitted. 8 submissions to Guardian were all rejected. Within-revision correction. |
